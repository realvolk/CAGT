...
/* GPUs: PCI device ID and display name */
static const struct gpu_entry known_gpus[] = {
    {"0x67df", "Radeon RX 580 2048SP (Polaris 20)"},
    {"0x6fdf", "Radeon RX 580 (Polaris 20)"},
    {"0x67ef", "Radeon RX 570 (Polaris 20)"},
    {"0x67ff", "Radeon RX 570 (Polaris 20)"},
    {"0x67c0", "Radeon RX 560 (Polaris 11)"},
    {"0x67c7", "Radeon RX 560 (Polaris 11)"},
    {"0x67e0", "Radeon RX 560 (Polaris 11)"},
    {"0x67e3", "Radeon RX 560 (Polaris 11)"},
    {"0x67e8", "Radeon RX 560 (Polaris 11)"},
    {"0x687f", "Radeon RX Vega 64"},
    {"0x6863", "Radeon RX Vega 56"},
    {"0x66af", "Radeon VII"},
    {"0x731f", "Radeon RX 5700 XT (Navi 10)"},
    {"0x7310", "Radeon RX 5700 (Navi 10)"},
    {"0x7312", "Radeon RX 5600 XT (Navi 10)"},
    {"0x73bf", "Radeon RX 6900 XT (Navi 21)"},
    {"0x73a5", "Radeon RX 6800 XT (Navi 21)"},
    {"0x73a3", "Radeon RX 6800 (Navi 21)"},
    {"0x73df", "Radeon RX 6700 XT (Navi 22)"},
    {"0x744c", "Radeon RX 7900 XTX (Navi 31)"},
    {"0x7448", "Radeon RX 7900 XT (Navi 31)"},
    {"0x7480", "Radeon RX 7800 XT (Navi 32)"},
    {"0x7484", "Radeon RX 7700 XT (Navi 32)"},
    {"0x74a0", "Radeon RX 7600 (Navi 33)"},
    {"0x679a", "Radeon HD 7950/70 (Tahiti)"},
    {"0x6798", "Radeon HD 7970 (Tahiti)"},
    {"0x6810", "Radeon R9 290/390 (Hawaii)"},
    {"0x67b0", "Radeon R9 290X/390X (Hawaii)"},
    {"0x7300", "Radeon R9 Fury (Fiji)"},
    {NULL, NULL}
};
    ...
    while ((e = readdir(d)) != NULL) {
        ...
        if (strstr(vendor, "0x1002")) {
            ...
            while ((he = readdir(hd)) != NULL) {
                ...
                snprintf(hwmon_path, sizeof(hwmon_path), "%s/%s/%s", path, he->d_name, "");
                hwmon_path[strlen(hwmon_path) - 1] = 0;
                closedir(hd);
                closedir(d);
                return 1;
            }
...
void draw_curve_info(WINDOW *win, int y, int x, int current_temp_c) {
    ...
    wattron(win, A_BOLD);
    mvwprintw(win, y, x, "Fan Curve (temp -> fan)");
    wattroff(win, A_BOLD);

    for (int i = 0; i < 8; i++) {
        int active = (current_temp_c >= points_c[i][0] &&
                     (i == 7 || current_temp_c < points_c[i+1][0]));
        double display_temp = convert_temp(points_c[i][0]);
...
void draw_ui(int temp_c, int speed) {
    ...
    mvprintw(5, 4, "Path:   %s", hwmon_path);

    move(7, 0);
    clrtoeol();
    wattron(stdscr, A_BOLD);
    mvprintw(7, 2, "Status  [%s]", temp_unit_names[temp_unit]);
    wattroff(stdscr, A_BOLD);
    ...
    draw_bar(stdscr, 12, 4, bar_width, (double)speed, 100.0, "Fan   ", "%");
    if (has_colors())
    ...
    draw_bar(stdscr, 14, 4, bar_width, temp_display, temp_max, temp_bar_label, "");
    if (has_colors())
        wattroff(stdscr, COLOR_PAIR(1) | COLOR_PAIR(2) | COLOR_PAIR(3));

    draw_curve_info(stdscr, 16, 4, temp_c);

    if (max_x > 60)
        draw_controls(stdscr, 16, 42);
    else
        draw_controls(stdscr, 26, 4);
...
int main() {
    ...
    endwin();
    restore_fan_auto();
    printf("[CAGT] Exited cleanly. Fan control restored to auto.\n");
    return 0;
}